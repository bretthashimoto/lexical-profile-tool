...
# ---------------------------------------------------------------------------
# Header banner (full width, above the two-column layout below)
# ---------------------------------------------------------------------------
_banner_html = f"""
<link rel="stylesheet"
...
# ---------------------------------------------------------------------------
# Two-column layout: reference builder (left, 20%) + tabs/results (right, 80%)
# ---------------------------------------------------------------------------
...
with col_right:
    ...
    with tab_profile:
        if not st.session_state.reference:
            st.info(
                "Build a reference on the left (or click **Load bundled example data**) "
                "to get started."
            )
        else:
...
can exclude either category (left panel, step 3) to report it separately
...
"""
    ...
    with tab_guide:
        ...
        st.markdown(
            """
1. **Build a reference** (left panel, step 1). Pick one:
...
2. **Add an ignore list (optional)** (left panel, step 2). Upload or paste
   specific words (names, made-up words, etc.) you don't want counted as
   off-list.

3. **Proper nouns & numerals (optional)** (left panel, step 3). By default
